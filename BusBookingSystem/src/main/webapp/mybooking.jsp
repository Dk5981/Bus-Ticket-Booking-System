...
<body>
	...
	int cnt = 0;
	%>
	<!-- busList shows all the records of buses it is comes from the MyBooking Servlet. -->
	<%
	List<Bus> busList = (List<Bus>) request.getAttribute("busList");
	%>
	<%
	List<Booking> bookingList = (List<Booking>) request.getAttribute("myBookingList");
	%>
	<%
	User userObj = (User) request.getAttribute("user");
	%>
	<%
	String bookTicketMsg = (String) request.getAttribute("plzBookTicket");
	%>
	<div class="container" style="border: t">
		<h2 style="text-align: center; margin-top: 20px;">My Bookings</h2>
		<br> <br>
		<%
		if (bookTicketMsg != null) {
		%>
		<h4 style="text-align: center; color: red; font-family: monospace;"><%=bookTicketMsg%></h4>
		<%
		} else {
		%>
